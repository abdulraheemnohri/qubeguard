...
object DnsTunPacketCodec {
    ...
    fun extractDnsQuery(packet: ByteArray): DnsPacket? {
        ...
        if (((packet[0].toInt() ushr 4) and 0x0F) != IPV4) return null
        ...
        return DnsPacket(src, dst, sourcePort, destinationPort, packet.copyOfRange(ihl + UDP_HEADER, ihl + udpLength))
    }

    fun buildDnsResponse(request: DnsPacket, dnsResponse: ByteArray): ByteArray {
        require(dnsResponse.isNotEmpty())
    ...
    private fun udpChecksum(packet: ByteArray, udpOffset: Int, udpLength: Int, src: Int, dst: Int): Int {
        ...
        sum += UDP; sum += udpLength
        var i = udpOffset
        val end = udpOffset + udpLength
        while (i + 1 < end) {
            if (i == udpOffset + 6) { i += 2; continue }
    ...
    private fun writeU32(data: ByteArray, offset: Int, value: Int) {
        data[offset] = (value ushr 24).toByte(); data[offset + 1] = (value ushr 16).toByte()
        data[offset + 2] = (value ushr 8).toByte(); data[offset + 3] = value.toByte()
    }
}
